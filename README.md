...
В этой ветке находится версия программы, которая используется для работы с реальными прототипами. Из внешних ADC работает только `ADC1`. Второму ADC на данный момент нет возможности назначить собственный DRDY пин.
...
- [Программа для считывания данных ЭКГ контроллером STM32G431CBU6](#программа-для-считывания-данных-экг-контроллером-stm32g431cbu6)
  ...
  - [Примерная схема стриминга данных (схема не актуальна)](#примерная-схема-стриминга-данных-схема-не-актуальна)
...
## Примерная схема стриминга данных (схема не актуальна)
...
1. Добавить свою функцию в перечисление `CommandID`, при желании назначить свое значение,
2. Дописать прототип функции к уже имеющимся в файле.
